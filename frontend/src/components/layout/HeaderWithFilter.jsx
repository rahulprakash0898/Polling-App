...
const HeaderWithFilter = ({title, filterType, setFilterType}) => {
  ...
  return (
    ...
    {open && (
        <div className='flex items-center gap-4 bg-primary p-4 rounded-l-lg rounded-b-lg'>
            {[{ label: 'All', value: '' }, ...POLL_TYPE].map((type)=>(
            <button
                key={type.value}
                className={`text-[12px] px-4 py-1 rounded-lg text-nowrap ${filterType == type.value
